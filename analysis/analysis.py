...
# This script will generate a list of nodes and edges with which to generate a directed force-directed graph
# The nodes are generated from a list of protein variants of interest
# The edges are drawn between nodes 1 hamming distance apart
# Edge direction is dictated by the order in which nodes are called for creating new edges
...
def calc_CAI(read1_file, read2_file, population2, min_count = 5, top = 100, bottom = 100):
    # This script iterates through reads for a given population and calculates the CAI for each unique read
    # Here, I restrict the analysis to the top/bottom user-defined number of reads for a given population
    ...
    # First, generating dictionaries from the Q-filtered, trimmed naive and PS4T5 reads files
    # Key will be the sequence, value will be the count
    # I also set up a dictionary of the reads and their translations for easy indexing
...
def calc_enrich(file1, file2, population1, population2, min_count):
    # Takes two .fastq residue files and calculates enrichment rates of pop2/pop1
...
def count_Stops(residue_file, population):
    # This script compiles the counts for all reads that contain at least 1 stop codon
    total_reads = 0
    stop_count = 0
    stop_dic = {}
